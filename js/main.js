...
// ============================================================
// ТЕКУЩИЕ ПРОЕКТЫ — карточки «в разработке»
// Тексты берутся из translations[lang].current_projects,
// проценты/теги/картинки — из currentProjects[] в i18n.js
// ============================================================
function buildCurrentProjects() {
    const t = translations[currentLang];
    const items = t.current_projects || translations.en.current_projects;
    const stack = document.getElementById('current-stack');
    if (!stack || !items) return;

    stack.innerHTML = '';

    items.forEach((item, i) => {
        const meta = currentProjects[i] || {};
        const pct = Number(meta.progress) || 0;

        const media = meta.image
            ? `<img src="${meta.image}" alt="${item.name}" loading="lazy">`
            : `<div class="current-image-empty">
           <span class="current-image-emoji">🎮</span>
           <span>${t.current_soon || translations.en.current_soon}</span>
         </div>`;

        const card = document.createElement('div');
        card.className = 'current-card reveal';
        card.innerHTML = `
      <div>
        <div class="current-badge">
          <span>⚡</span>
          <span>${t.current_badge}</span>
        </div>
        <h3 class="current-name">${item.name}</h3>
        <p class="current-desc">${item.desc}</p>

        <div class="progress-wrap">
          <div class="progress-label">
            <span>${t.current_progress}</span>
            <span>${pct}%</span>
          </div>
          <div class="progress-bar">
            <div class="progress-fill" data-progress="${pct}" style="width:0%"></div>
          </div>
        </div>

        <div class="current-tags">
          ${(meta.tags || []).map(tag => `<span class="current-tag">${tag}</span>`).join('')}
        </div>
      </div>

      <div class="current-image-wrap${meta.image ? ' has-image' : ''}">${media}</div>
    `;
...
function updateEngineLink() {
    const a = document.getElementById('engine-link');
    if (a && typeof engineLink === 'string' && engineLink) a.href = engineLink;
}

// ============================================================
// ПРОГРЕСС-БАРЫ — заполняются при попадании в кадр
// ============================================================
...
function initProgressBars() {
    ...
    progressObserver = new IntersectionObserver(entries => {
        entries.forEach(e => {
            if (e.isIntersecting) {
                e.target.style.width = (e.target.dataset.progress || 0) + '%';
